...
function editRow() {
    ...
    calories[editIndex] = cInput.value;
    fillTable();
    addToLocalStorage();
    cancelEdit();
}
...
function deleteRow(i) {
    if (!confirm(`Confirm that you want to delete the entry: \n ${date[i]}: ${water[i]}ml, ${exercise[i]}min, ${bloodsugar[i]}mg/dL, ${calories[i]}kcal`))
        return;
    date.splice(i, 1);
    water.splice(i, 1);
    exercise.splice(i, 1);
...
let calories = JSON.parse(localStorage.getItem("calories")) || [];
...
submitButton.addEventListener("click", () => {
    ...
    showEncouragingPopup(c);
    clearInputs();
    fillTable();
    addToLocalStorage();
});

function calculateCalories(weight, duration) {
    const MET = 7.0;  // Average MET value for moderate exercise (e.g., jogging)
    return (weight * duration * MET) / 60; // Calories burnt per minute of exercise
}

function showEncouragingPopup(calories) {
    let message;

    if (calories < 200) {
        message = "Good start! Every step counts. Keep it up! 💪";
    } else if (calories >= 200 && calories <= 500) {
        message = "Great job! You're on your way to fitness success! 🏃‍♂️✨";
    } else {
        message = "Amazing effort! You're smashing your goals! 🔥🙌";
    }
